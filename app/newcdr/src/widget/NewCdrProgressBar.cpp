...
static void my_draw_progress (HWND hWnd, HDC hdc, int nMax, int nMin, int nPos)
{
	...
	y = RECTH (rcClient)*2/5 + pbar_border;
	w = RECTW (rcClient) - (pbar_border << 1);
	h = RECTH (rcClient)/5 - (pbar_border << 1);

	//SetWindowBkColor(hWnd, RGBA2Pixel(hdc, 0x00, 0x00, 0x00, 0x00));
	if (hWnd != HWND_NULL)
		old_color = SetBrushColor (hdc, RGBA2Pixel(hdc, 0xaf, 0xaf, 0xaf, 0x00));
	else
		old_color = SetBrushColor (hdc, GetWindowElementPixel (HWND_DESKTOP, WE_BGC_DESKTOP));

	//draw the erase background
	FillBox (hdc, rcClient.left, RECTH (rcClient)*2/5, RECTW (rcClient), RECTH (rcClient)/5);
	...
	case MSG_CREATE:
		{
			...
			retWnd = CreateWindowEx(CTRL_STATIC, NULL,
					WS_VISIBLE | SS_SIMPLE | SS_VCENTER | SS_CENTER,
