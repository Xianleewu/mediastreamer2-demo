/*************************************************************************
  ...
  > Created Time: 2014年04月20日 星期日 19:03:43
 ************************************************************************/
...
#ifdef USE_NEWUI
	if (GetWindowStyle (hwnd) & LBS_SBNEVER)
		pData->sbPolicy = SB_POLICY_NEVER;
#else
...
#ifdef USE_NEWUI	
	if (pData->title) {
		prc->top += pData->itemHeight;
		prc->bottom += pData->itemHeight;
	}
#endif
}
...
#ifdef USE_NEWUI
	if (pData->title) {
		rcInv.top += pData->itemHeight;
		rcInv.bottom += pData->itemHeight;
	}
#endif
...
#ifdef USE_NEWUI
	if (pData->title)
		pData->itemVisibles = pData->itemVisibles - 1; 
#endif
...
static void lstOnDrawSListBoxItems (HWND hWnd, HDC hdc, PLISTBOXDATA pData)
{
	PLISTBOXITEM plbi;
	PCONTROL pWin;
	RECT rect, tempRect;
	...
	if (pData->title) {
		...
		tempRect.bottom = pData->itemHeight;

		if (pData->pBgPic)
			FillBitmapPartInBox(hdc, 0, 0, width, pData->itemHeight,
					pData->pBgPic, 2, 2, pData->pBgPic->bmWidth-2, pData->pBgPic->bmHeight/2);
		DrawText (hdc, pData->title, -1, &tempRect, DT_TOP | DT_CENTER | DT_SINGLELINE | DT_VCENTER);

		if (pData->pBgPic) {
			FillBitmapPartInBox(hdc, 0, pData->itemHeight, width, RECTH(rect)-pData->itemHeight-10,
					pData->pBgPic, 2, (pData->pBgPic->bmHeight/2)+10, 
					pData->pBgPic->bmWidth-2, (pData->pBgPic->bmHeight/2)-20);
			FillBitmapPartInBox(hdc, 0, RECTH(rect)-10, width, 10,
					pData->pBgPic, 2, pData->pBgPic->bmHeight-10,
					pData->pBgPic->bmWidth-2, 10);
		}
		y = pData->itemHeight;
	}
#endif	
	for (i = 0; plbi && i < (pData->itemVisibles + 1); i++) {
		...
		if (plbi->dwFlags & LBIF_SELECTED) {
			...
			if (pData->pLightBgPic) {
				/** render hilited item */
				FillBoxWithBitmap(hdc, tempRect.left, tempRect.top, RECTW(tempRect), RECTH(tempRect), pData->pLightBgPic);
			}
#else
...
#ifdef USE_NEWUI
		if (pWin->dwStyle & LBS_SCENTER)
			DrawText (hdc, plbi->key, -1, &rect, DT_TOP | DT_CENTER | DT_SINGLELINE | DT_VCENTER);
		else
#endif
...
int MenuListCallback(HWND hwnd, int message, WPARAM wParam, LPARAM lParam)
{
...
#ifdef USE_NEWUI
			if (menuListAttr->title) {
#if _USE_FIXSTR
				len = strlen (menuListAttr->title);
				pData->title = FixStrAlloc (len);
				if (!pData->title) {
					return LB_ERRSPACE;
				}
				if (len > 0)
					strcpy (pData->title, menuListAttr->title);
#else
				pData->title = strdup (menuListAttr->title);
				if (!pData->title) {
					return LB_ERRSPACE;
				}
#endif
			}
			pData->pBgPic = menuListAttr->mlBgpic;
			pData->pLinePic = menuListAttr->mlLinePic;
			pData->pLightBgPic = menuListAttr->mlLightBgPic;
			pData->pSelectIcon = menuListAttr->mlSelectIcon;
#endif
	...
	case MSG_PAINT:
		{
			...
			lstOnDrawSListBoxItems (hwnd, hdc, pData);
			
			#ifdef USE_NEWUI
			if (pData->pLinePic) {
				if (pData->itemVisibles>=2)
					for( count = 2; count <= pData->itemVisibles; count++ )
						FillBoxWithBitmap(hdc, 2, (count*pData->itemHeight)-1, RECTW(rc)-4, 1, pData->pLinePic);
			}
			#else
			...
			for( count = 1; count <= pData->itemCount; count++ ) {
				LineEx(hdc, 0, pData->itemHeight * count, RECTW(rc), pData->itemHeight * count);
			}
			#endif
	...
	case LB_SETSELECT:
		{
			int iSelect = (int)lParam;
			pData = (PLISTBOXDATA)pCtrl->dwAddData2;
			if (iSelect >=0 )
				pData->itemSelect = iSelect;
			return 0;
		}
	case LB_GETSELECT:
		{
			pData = (PLISTBOXDATA)pCtrl->dwAddData2;
			return pData->itemSelect;
		}
	case LB_SETTITLE:
		{
			RECT rcInv;
			...
			if (newStr) {
				...
				pData->title = newStr;
				if (len > 0)
					strcpy (pData->title, (char*)lParam);
			}
			else
				return LB_ERR;
#else
...
#ifdef USE_NEWUI
			if (pData->title) {
				rcItem->top += pData->itemHeight;
				rcItem->bottom += pData->itemHeight;
			}
#endif
			return LB_OKAY;
		}    
