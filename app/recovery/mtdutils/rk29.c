...
int make_vfat(const char *filename,const char* volumelabel)
{
    ...
    if(volumelabel == NULL){
         const char *const mke2fs_argv[] = { "/sbin/newfs_msdos", "-F", "32", filename, NULL };
         return run(mke2fs_argv[0], (char **) mke2fs_argv);
    }else{
         const char *const mke2fs_withLabel_argv[] = { "/sbin/newfs_msdos", "-F", "32", "-L", volumelabel, filename, NULL };
