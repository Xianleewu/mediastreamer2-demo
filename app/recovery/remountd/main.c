...
int main(int argc, char **argv)
{
    ...
    if (remount("/system", &system_ro)) {
        printf("remount of system failed: %s\n",strerror(errno));
    }
    
   
    fd = open("/system/vendor/firmware/remout4.txt", O_RDWR|O_CREAT, 0666);
    if(fd < 0)
    {
    	printf("[jdy] create file failed: %s\n",strerror(errno));
    	ALOGE("[%s:%d] remount service ", __func__, __LINE__);
    }
    else
    {
    	write(fd, "hello,remount", strlen("hello,remount"));
    	close(fd);
    }
